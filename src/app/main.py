...
def load_data():
    ...
    return df_tickets, df_tec

df_tickets, df_tec = load_data()
...
lista_loc = ["Todas"] + sorted(df_filtrado['localizacao'].dropna().unique().tolist())
loc_sel = st.sidebar.selectbox("Localização", lista_loc)
if loc_sel != "Todas":
    df_filtrado = df_filtrado[df_filtrado['localizacao'] == loc_sel]
...
tab1, tab2, tab3 = st.tabs(["📋 Visão Detalhada", "📈 Comparativos (MoM/YoY)", "🏢 Análise por Localização"])
...
with tab1:
    ...
    st.dataframe(df_filtrado[['id', 'titulo', 'categoria', 'localizacao', 'data_abertura', 'tecnicos']], width='stretch', hide_index=True)
...
with tab3:
    st.header("Análise por Localização e Técnicos")

    df_tec_completo = pd.merge(df_tec_filtrado, df_filtrado[['id', 'categoria', 'localizacao']], left_on='ticket_id', right_on='id', how='left')

    c_loc, c_tec = st.columns(2)

    with c_loc:
        st.subheader("Top Localizações (Volume)")
        if not df_filtrado.empty:
            df_loc_counts = df_filtrado['localizacao'].value_counts().head(10).reset_index()
            fig_loc = px.bar(df_loc_counts, x='count', y='localizacao', orientation='h', labels={'count': 'Chamados', 'localizacao': 'Localização'})
            fig_loc.update_yaxes(autorange="reversed", type='category')
    ...
    st.markdown("### 🔍 Perfil de Categorias por Localização e Técnico (Top 3)")
    c_det_loc, c_det_tec = st.columns(2)

    with c_det_loc:
        st.markdown("**Selecione uma Localização:**")
        lista_locs = sorted(df_filtrado['localizacao'].dropna().unique()) if not df_filtrado.empty else []
        if lista_locs:
            loc_especifica = st.selectbox("Localização", lista_locs, label_visibility="collapsed")
            df_loc_ind = df_filtrado[df_filtrado['localizacao'] == loc_especifica]
            top3_cat_loc = df_loc_ind['categoria'].value_counts().head(3).reset_index()
            fig_pie_loc = px.pie(top3_cat_loc, values='count', names='categoria', hole=0.3, title=f"Top 3 - {loc_especifica}")
            fig_pie_loc.update_traces(textposition='inside', textinfo='percent+label')
            st.plotly_chart(fig_pie_loc, width='stretch')
